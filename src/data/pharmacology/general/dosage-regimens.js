export const lessons = [
{
    ...
    keyPoints:['Loading dose mainly relates to Vd and target concentration','Maintenance dosing mainly relates to clearance','Route and bioavailability influence the administered dose'],rapid:['Loading → reach target','Maintenance → replace elimination'],
    quiz:[{question:'A loading dose is calculated primarily based on target plasma concentration (Cp) and which pharmacokinetic parameter?',options:['Renal clearance (CL)','Apparent volume of distribution (Vd)','Elimination rate constant (ke)','Urinary excretion fraction (fe)'],answer:1,explanation:'Loading dose (LD = [Cp × Vd] / F) is designed to immediately fill the volume of distribution to reach target therapeutic levels rapidly.'}]
  },
{
    ...
    keyPoints:['Dose = amount','Dose rate = amount/time','Concentration = amount/volume','Units are part of the answer'],rapid:['mg = amount','mg/h = rate','mg/L = concentration'],
    quiz:[{question:'Maintenance dose rate at steady state is determined primarily by target concentration (Cp) and:',options:['Total body clearance (CL)','Volume of distribution (Vd) alone','Receptor affinity constant (Kd)','Peak plasma concentration (Cmax)'],answer:0,explanation:'Maintenance dosing rate (MD rate = [Cp × CL] / F) replaces drug lost through clearance to maintain a constant average steady-state concentration.'}]
  }
];
